...
// 2. UPDATE MOOD DATA TO USE ICON NAMES INSTEAD OF EMOJIS
const MOODS = [
  { key: 'left-spacer' },
  { 
    id: '1', 
    ...
    color: '#FF6B6B', 
    score: 1, 
    message: "Take a deep breath. It's okay to feel this way." 
  },
  { 
    id: '2', 
    ...
    color: '#4ECDC4', 
    score: 2, 
    message: "You are not alone. Be gentle with yourself." 
  },
  { 
    id: '3', 
    ...
    color: '#FFE66D', 
    score: 3, 
    message: "A balanced state is a good place to be." 
  },
  { 
    id: '4', 
  ...
  { 
    ...
    color: '#FF9F1C', 
    score: 5, 
    message: "That's fantastic! Hold onto this feeling." 
  },
  { key: 'right-spacer' },
];
...
const MoodTrackerScreen = () => {
  ...
  const [selectedMood, setSelectedMood] = useState(MOODS[3]); // Default to Good
    ...
    return (
      ...
      >
        <View style={[
          ...
          isSelected && { 
            transform: [{ scale: 1.3 }], // Slightly reduced scale for cleaner look
            backgroundColor: item.color + '15', // Very light background
            borderColor: item.color,
            borderWidth: 2
          }
        ]}>
          {/* 3. RENDER ICON INSTEAD OF TEXT */}
          <MaterialCommunityIcons 
            name={item.icon} 
            size={40} 
            // If selected, use specific mood color. If not, use secondary text color (grey)
            color={isSelected ? item.color : theme.secondaryText} 
          />
        </View>
      </TouchableOpacity>
    );
...
const styles = (theme) => StyleSheet.create({
  ...
  emojiContainer: {
    width: 80, height: 80, borderRadius: 40, // Increased size slightly
    alignItems: 'center', justifyContent: 'center',
    backgroundColor: 'transparent',
  },
  // Removed .emoji style as we are using Icons now
